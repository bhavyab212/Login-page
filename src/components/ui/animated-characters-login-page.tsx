...
function LoginPage({ onNavigate, onLoginSuccess }: LoginPageProps) {
    ...
    const calculatePosition = (ref: React.RefObject<HTMLDivElement | null>) => {
        ...
        // Reduced X divisor from 20 to 25 for smoother horizontal movement
        // Reduced Y divisor from 30 to 20 for better vertical responsiveness
        const faceX = Math.max(-15, Math.min(15, deltaX / 25));
        const faceY = Math.max(-10, Math.min(10, deltaY / 20));

        // Body lean with smoother response (reduced from 120 to 100)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Basic validation - accept any email/password combination
        if (email && password) {
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-2">
                                    <Checkbox id="remember" className={theme === 'light' ? 'border-violet-300 data-[state=checked]:bg-violet-600' : ''} />
                                    <Label
                                        htmlFor="remember"
                                        className={`text-sm font-extrabold cursor-pointer transition-colors duration-200
                                ...
                                <a
                                    href="#"
                                    className={`text-sm font-extrabold hover:underline transition-colors duration-200
                                        ${theme === 'dark'
                            ...
                            <Button
                                ...
                                disabled={isLoading}
                            >
                                <span className="relative z-10">{isLoading ? "Signing in..." : "Log in"}</span>
